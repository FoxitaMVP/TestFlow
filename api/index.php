...
function ensureSupplementalSchema(PDO $pdo, string $driver): void
{
    if ($driver === 'sqlite') {
        ...
        addColumnIfMissing($pdo, 'sqlite', 'users', 'teams_email', 'TEXT');
        ...
        addColumnIfMissing($pdo, 'sqlite', 'test_case_steps', 'error_saved_at', 'INTEGER');
        $pdo->exec(
    ...
    addColumnIfMissing($pdo, 'mysql', 'users', 'teams_email', 'VARCHAR(190) NULL');
    ...
    addColumnIfMissing($pdo, 'mysql', 'test_case_steps', 'error_saved_at', 'BIGINT NULL');
    $pdo->exec(
        'CREATE TABLE IF NOT EXISTS test_case_assignees (
...
{
    $users = $pdo->query('SELECT id, name, email, password_hash, role, status, requested_at, rejected_at, teams_email, telegram_url, active_session_token, last_activity_at FROM users ORDER BY created_at, id')->fetchAll();
...
function fetchCases(PDO $pdo): array
{
    ...
    $stepsByCase = [];
    $steps = $pdo->query(
        'SELECT id, test_case_id, precondition, action, expected_result, actual_result, comment, result_status, error_guid, error_saved_at
    ...
    foreach ($steps as $step) {
        $stepsByCase[$step['test_case_id']][] = [
            ...
            'errorSavedAt' => isset($step['error_saved_at']) ? (int) $step['error_saved_at'] : null,
        ];
    }
...
{
    $stmt = $pdo->prepare('INSERT INTO users (id, name, email, password_hash, role, status, requested_at, rejected_at, teams_email, telegram_url, active_session_token, last_activity_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
...
function saveCases(PDO $pdo, array $cases): void
{
    ...
    $stepStmt = $pdo->prepare(
        'INSERT INTO test_case_steps
         (id, test_case_id, sort_order, precondition, action, expected_result, actual_result, comment, result_status, error_guid, error_saved_at)
         VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)'
    );
